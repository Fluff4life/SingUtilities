...
client.on('ready',  async() => {
    ...
    const statusArray = ['the news, LISTENING', `SingPiano, LISTENING` , 'Mods in the closet, WATCHING' , 'for rule breakers,  WATCHING', `Sakura SMP, WATCHING` , 'Giveaways, WATCHING', 'COVID-19, WATCHING' ,`#main-chat, WATCHING`, 'You, WATCHING', `Minecraft, PLAYING`]
    setInterval(() => {
...
if(msg.content === "!verify" && msg.channel.id === "1127926228104515674") { //ADD YOUR VERIFY CHANNEL ID HERE
    msg.delete()
...
if(msg.mentions.users.size > 4 && !msg.member.hasPermission('MANAGE_MESSAGES')) {
    var emojiGuild = client.guilds.cache.find(guild => guild.name === '1127926646100471849')//ADD YOUR CHANNEL NAME HERE
    var animebonk = emojiGuild.emojis.cache.find(emoji => emoji.name === 'animebonk')
